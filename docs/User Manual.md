...
and continue parsing. In relaxed mode the parser returns a tuple representing the
...
>>> from flanker.addresslib import address
>>>
>>> address.validate_list('foo@mailgun.com, bar@mailgun.com, @mailgun.com', False)
...
messages. `flanker.mime` is is faster (~20x) and more memory efficient than the
standard Python MIME parser. The parser also attempts to preserve encodings when
possible. The parser only operates under strict mode only (does not attempt to
fix any issues other than simple errors).

#### Rationale

Mailgun parses a lot of MIME, and therefore requires a fast and memory efficient
MIME handling package. Below is a comparison between the default Python MIME parser and
`flanker.mime`.

| `email.parser`                           | `flanker.mime`                            |
| ---------------------------------------- | ----------------------------------------- |
| Takes ~1 second to parse a 11MB message. | Takes ~50 millisecond to parse 11 MB message. |
| Takes a couple of million ops to parse a 11MB message. | Takes ~2K operations to parse 11 MB message. |
...
Because the parser only operates in strict mode, when it encounters broken MIME, it
raises a MimeError and does not attempt to fix anything except simple errors 
(like mistyped charsets)

If you still need to process a broken MIME message, you can use 
`flanker.mime.fallback.FallbackMessage` which relies on the standard Python parser `email.parser`
to fix the broken MIME and force broken encodings in bodies and headers. However, beware that it
