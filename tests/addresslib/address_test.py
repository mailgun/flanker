...
def test_addresslist_with_apostrophe():
    ...
    lst = parse_list(s)
    eq_ (2, len(lst))
    eq_ ('"Allan Nasong\'o" <nasongoallan@yahoo.com>', lst[0].full_spec())
    eq_ ('"Osozi Wilfred" <osoziwilfred@yahoo.com>', lst[1].full_spec())
...
def test_edge_cases():
    email = EmailAddress('"catherine.emerson@"@va.gov')
    eq_('"catherine.emerson@"@va.gov', email.address)
